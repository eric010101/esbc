...
// Create page object
$sensor_basic_add = new sensor_basic_add();

// Run the page
$sensor_basic_add->run();
...
var fsensor_basicadd = currentForm = new ew.Form("fsensor_basicadd", "add");

// Validate form
fsensor_basicadd.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($sensor_basic_add->sensor_id->Required) { ?>
		...
		<?php } ?>
		<?php if ($sensor_basic_add->beb_alias_name->Required) { ?>
		...
		<?php } ?>
		<?php if ($sensor_basic_add->active->Required) { ?>
		...
		<?php if ($sensor_basic_add->date_add->Required) { ?>
...
fsensor_basicadd.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
fsensor_basicadd.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php $sensor_basic_add->showPageHeader(); ?>
<?php
$sensor_basic_add->showMessage();
?>
<form name="fsensor_basicadd" id="fsensor_basicadd" class="<?php echo $sensor_basic_add->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($sensor_basic_add->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $sensor_basic_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="sensor_basic">
<input type="hidden" name="action" id="action" value="insert">
<input type="hidden" name="modal" value="<?php echo (int)$sensor_basic_add->IsModal ?>">
<div class="ew-add-div"><!-- page* -->
<?php if ($sensor_basic->sensor_id->Visible) { // sensor_id ?>
	<div id="r_sensor_id" class="form-group row">
		<label id="elh_sensor_basic_sensor_id" for="x_sensor_id" class="<?php echo $sensor_basic_add->LeftColumnClass ?>"><?php echo $sensor_basic->sensor_id->caption() ?><?php echo ($sensor_basic->sensor_id->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sensor_basic_add->RightColumnClass ?>"><div<?php echo $sensor_basic->sensor_id->cellAttributes() ?>>
...
<?php if ($sensor_basic->beb_alias_name->Visible) { // beb_alias_name ?>
	<div id="r_beb_alias_name" class="form-group row">
		<label id="elh_sensor_basic_beb_alias_name" for="x_beb_alias_name" class="<?php echo $sensor_basic_add->LeftColumnClass ?>"><?php echo $sensor_basic->beb_alias_name->caption() ?><?php echo ($sensor_basic->beb_alias_name->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sensor_basic_add->RightColumnClass ?>"><div<?php echo $sensor_basic->beb_alias_name->cellAttributes() ?>>
...
<?php if ($sensor_basic->active->Visible) { // active ?>
	<div id="r_active" class="form-group row">
		<label id="elh_sensor_basic_active" for="x_active" class="<?php echo $sensor_basic_add->LeftColumnClass ?>"><?php echo $sensor_basic->active->caption() ?><?php echo ($sensor_basic->active->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sensor_basic_add->RightColumnClass ?>"><div<?php echo $sensor_basic->active->cellAttributes() ?>>
...
<?php if ($sensor_basic->date_add->Visible) { // date_add ?>
	<div id="r_date_add" class="form-group row">
		<label id="elh_sensor_basic_date_add" for="x_date_add" class="<?php echo $sensor_basic_add->LeftColumnClass ?>"><?php echo $sensor_basic->date_add->caption() ?><?php echo ($sensor_basic->date_add->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $sensor_basic_add->RightColumnClass ?>"><div<?php echo $sensor_basic->date_add->cellAttributes() ?>>
...
<script>
ew.createDateTimePicker("fsensor_basicadd", "x_date_add", {"ignoreReadonly":true,"useCurrent":false,"format":1});
</script>
<?php } ?>
</span>
...
<?php if (!$sensor_basic_add->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $sensor_basic_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $sensor_basic_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$sensor_basic_add->showPageFooter();
if (DEBUG_ENABLED)

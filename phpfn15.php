...
function CurrentProjectID() {
	...
	return "{04503673-A70B-421F-8E02-1DFE5D0B56A3}";
}
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						if ($rs->fields('USER_LEVEL') == NULL) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID((int)$rs->fields('USER_LEVEL')); // Load User Level
						}
						$this->setupUserLevel();
...
function Database_Connecting(&$info) {
	if ($info["id"] == "DB" && strpos(ServerVar("SERVER_NAME"), "hkvstore.com") !== FALSE) { // demo site
		$info["host"] = "localhost";
		$info["user"] = "gogocode_IOTadm";
		$info["pass"] = "sweetekhappy";
		$info["db"] = "gogocode_esbc";
	}
	if ($info["id"] == "demo2" && strpos(ServerVar("SERVER_NAME"), "hkvstore.com") !== FALSE) { // demo site
		$info["host"] = "localhost";
		$info["user"] = "gogocode_IOTadm";
		$info["pass"] = "sweetekhappy";
		$info["db"] = "gogocode_esbc";
